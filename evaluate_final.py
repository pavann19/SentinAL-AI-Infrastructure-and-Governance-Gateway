...
from core.config import CAPABILITY_GENERAL

# Configuration
#TEST_FILE = "tests/test_prompts.json"
TEST_FILE = "data/adversarial_dataset_v1.json"
...
def run_evaluation():
    ...
    benign_prompts = [item["prompt"] for item in data if item["type"] in ("benign", "benign_adversarial")]
    attack_prompts = [item["prompt"] for item in data if item["type"] == "malicious"]
